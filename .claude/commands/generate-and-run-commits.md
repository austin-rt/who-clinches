...
Execute `git add` and `git commit` commands for all unstaged changes. Analyze changes with `git diff`, determine optimal commit structure, then run commands using `run_terminal_cmd` with `required_permissions: ["all"]`.
