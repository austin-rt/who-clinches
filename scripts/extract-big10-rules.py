#!/usr/bin/env python3
"""
Extract Big Ten Tiebreaker Rules from PDF

Fetches the Big Ten tiebreaker rules PDF from bigten.org, extracts text content,
cleans PDF artifacts (page numbers, form feeds, excessive whitespace), and saves
it to docs/tiebreaker-rules/big10-tiebreaker-rules.txt for AI agent reference.

Usage:
    python scripts/extract-big10-rules.py
...
BIG10_WEB_PAGE = "https://bigten.org/fb/article/blt6104802d94ebe1ab/"
BIG10_PDF_URL = "https://bigten.org/api/media/file/85192401-602f-47ca-82c8-e7f8c97ef0d2-2024_Big_Ten_Football_Tiebreaker_-_FINAL_10__72_.pdf"
TEMP_DIR = Path(__file__).parent.parent / "temp"
OUTPUT_DIR = Path(__file__).parent.parent / "docs" / "tiebreaker-rules"
TEMP_PDF = TEMP_DIR / "big10-rules.pdf"
OUTPUT_TXT = OUTPUT_DIR / "big10-tiebreaker-rules.txt"
...
def main():
    ...
    print("Who Clinches - Big Ten Tiebreaker Rules Extraction Script")
    print("=" * 50)
    ...
    # Try direct PDF URL first, then fall back to web page parsing
    pdf_urls = [BIG10_PDF_URL]
    pdf_downloaded = False
    
    # Try direct PDF URL
    print(f"\nAttempting to download PDF from direct URL...")
    try:
        download_pdf(BIG10_PDF_URL, TEMP_PDF)
        print(f"✓ PDF downloaded to {TEMP_PDF}")
        pdf_downloaded = True
    except RuntimeError as e:
        print(f"  Direct URL failed: {e}")
        print("  Attempting to find PDF URL from web page...")
        
        # Fall back to web page parsing
        try:
            html = fetch_html(BIG10_WEB_PAGE)
            print("✓ HTML fetched")
            
            pdf_url = find_pdf_url(html, BIG10_WEB_PAGE)
            if pdf_url:
                ...
                try:
                    ...
                    print(f"ERROR: Failed to download PDF from {pdf_url}: {e2}")
                    sys.exit(1)
            else:
                print("ERROR: No PDF URL found in web page")
                sys.exit(1)
        except RuntimeError as e:
            print(f"ERROR: {e}")
            sys.exit(1)
    ...
    if not pdf_downloaded:
        print("ERROR: Failed to download PDF from all sources")
        sys.exit(1)
