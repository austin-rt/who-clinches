...
function findOrphanedDocs() {
  ...
  const orphaned = Array.from(allFilePaths).filter(f => 
    !referencedFiles.has(f) && !indexFiles.has(f)
  );
...
function reportTier1Load() {
  ...
  console.log(`\n  Note: ai-loading-manifest.md estimates ~5.5K tokens`);
  console.log(`  Status: ${totalTokensEst <= 5500 ? '✅ Within expected range' : '⚠️  Exceeds estimate'}`);
...
function main() {
  ...
  console.log(`Filename conventions: ${results.filenameConventions ? '✅ All valid' : '⚠️  Violations found'}`);
  ...
  const allPassed = results.brokenLinks && 
                    results.filenameConventions && 
                    results.duplicateHeadings;
