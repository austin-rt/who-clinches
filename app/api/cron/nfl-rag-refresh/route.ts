...
import {
  fetchNflSourceText,
  NFL_SOURCE_CONFIG,
  type NflStaticSource,
} from '@/lib/nfl/nfl-static-fetcher';
...
const updateSource = async (
  source: NflStaticSource
): Promise<{ chunks: number; skipped: boolean }> => {
  const { sourceFile } = NFL_SOURCE_CONFIG[source];
  const text = await fetchNflSourceText(source);
  ...
  const batchId = `cron-nfl-${source}-${Date.now()}`;
...
export const GET = async (request: NextRequest) => {
  ...
  const singleSource = request.nextUrl.searchParams.get('source') as NflStaticSource | null;
  ...
  for (const source of targets) {
    try {
      ...
      await logError(err, {
        endpoint: '/api/cron/nfl-rag-refresh',
        action: 'nfl-rag-cron',
        source,
      });
    }
  ...
  if (!allSkipped) {
    void sendEmail({
      from: 'rag',
      subject: `[Cron] NFL RAG refresh ${errors.length ? 'partial' : 'complete'}`,
      html: notificationHtml(
        `NFL RAG Refresh ${errors.length ? '(Partial)' : 'Complete'}`,
