...
export const GET = async (request: NextRequest) => {
  ...
  try {
    ...
    // 3. Check if we should update all games or just incomplete ones
    // Query param `allGames=true` updates all games (for daily batch)
    // Default behavior (allGames=false or missing) only updates incomplete games (for frequent updates during game windows)
    const { searchParams } = new URL(request.url);
    const allGames = searchParams.get('allGames') === 'true';

    // 4. Query DB for games
    // If allGames=true: get all games in current week
    // If allGames=false: only get incomplete games (default behavior)
    const query: {
      season: number;
    ...
    if (!allGames) {
      // Default: only incomplete games (for Pro plan frequent updates)
      query.completed = false;
    } else {
      // Batch mode: find current week first, then get all games in that week
      ...
      if (latestGame && typeof latestGame.week === 'number') {
        query.week = latestGame.week;
      }
    }

    const gamesRaw = await Game.find(query).lean();
    ...
    // 5. Early exit if no games found (no ESPN calls!)
    if (games.length === 0) {
    ...
    // 6. Get week number from first game (all games should be in same week)
    ...
    // 7. Fetch scoreboard from ESPN (one call for entire week)
    let espnResponse;
    try {
