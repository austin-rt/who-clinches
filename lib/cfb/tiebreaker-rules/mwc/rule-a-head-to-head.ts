...
export const applyRuleAHeadToHead = (
  ...
  if (!allAreCommonOpponents) {
    return { winners: tiedTeams, detail: 'Not all teams played each other - group remains tied' };
  }
  ...
  const eliminated = records
    .filter((r) => Math.abs(r.winPct - minWinPct) < EPSILON_CONSTANT)
    .map((r) => r.teamId);

  if (eliminated.length < tiedTeams.length) {
    const remaining = tiedTeams.filter((t) => !eliminated.includes(t));
    const eliminatedAbbrevs = eliminated.map((tid) => getTeamAbbrev(tid, games));
    const detail = `Mini round-robin: ${(maxWinPct * 100).toFixed(1)}% win pct. Eliminated: ${eliminatedAbbrevs.join(', ')}`;
    return { winners: remaining, detail };
  }
